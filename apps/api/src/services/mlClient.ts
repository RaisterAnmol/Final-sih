...
export class MLServiceClient {
  static async isHealthy(): Promise<boolean> {
    try {
      const resp = await axios.get(`${ML_SERVICE_URL}/health`, {
        timeout: 2000,
      });
  ...
  ): Promise<{ results: AnalysisOutput[]; metrics?: any }> {
    try {
        ...
        `${ML_SERVICE_URL}/analyze/batch`,
        {
          projects: projects.map((p) => ({
            ...
            actualCompletionDate: p.actualCompletionDate
              ? new Date(p.actualCompletionDate).toISOString().split("T")[0]
              : undefined,
            latitude: p.latitude,
      ...
      console.warn(
        `[ML Service] FastAPI connection unavailable (${err.message}). Using resilient local statistical engine.`,
      );
      const fallbackResults = FallbackRuleEngine.analyzeProjects(
        projects,
        customWeights,
      );
