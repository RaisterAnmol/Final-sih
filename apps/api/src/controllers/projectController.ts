...
export async function getProjects(req: Request, res: Response): Promise<void> {
  ...
  if (search) {
    ...
    const rawQ = (search as string).trim();
    const safeRegex = rawQ.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    filter.$or = [
      ...
      { projectId: { $regex: safeRegex, $options: "i" } },
      { title: { $regex: safeRegex, $options: "i" } },
      { mpName: { $regex: safeRegex, $options: "i" } },
      { constituency: { $regex: safeRegex, $options: "i" } },
      { ida: { $regex: safeRegex, $options: "i" } },
      { implementingAgency: { $regex: safeRegex, $options: "i" } },
      { contractorName: { $regex: safeRegex, $options: "i" } },
      { district: { $regex: safeRegex, $options: "i" } },
      { state: { $regex: safeRegex, $options: "i" } },
...
export async function getProjectById(
  ...
  let project = await Project.findOne({ $or: query });

  if (!project) {
    // Attempt case-insensitive regex lookup on projectId
    project = await Project.findOne({
      projectId: { $regex: new RegExp(`^${id}$`, "i") },
    });
  }
  ...
  res.json({
    ...
    data: {
      ...
      peerComparison: {
        ...
        peerAvgCost: Math.round(
          peerStats[0]?.avgCost || project.allocatedAmount,
        ),
        costRatio: peerStats[0]?.avgCost
          ? Number((project.allocatedAmount / peerStats[0].avgCost).toFixed(2))
          : 1.0,
      ...
      contractorProfile: {
        ...
        highRiskWorks:
          contractorHistory[0]?.highRiskWorks ||
          (project.riskLevel === "HIGH" ? 1 : 0),
      },
    },
  });
}
...
export async function exportProjectsCSV(
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="mplad_projects_export.csv"',
  );
  res.send(csvContent);
}
...
export async function runAnalysis(req: Request, res: Response): Promise<void> {
  ...
  if (projects.length === 0) {
    res.status(400).json({
      ...
      error: {
        code: "NO_PROJECTS",
        message: "No projects match query for analysis",
      },
    });
    return;
  }
  ...
  for (const r of analysis.results) {
    ...
    for (const s of r.signals) {
      anomalyDocs.push({
        ...
        projectId: r.projectId,
        projectTitle:
          projects.find((p) => p.projectId === r.projectId)?.title || "",
        state: projects.find((p) => p.projectId === r.projectId)?.state || "",
        district:
          projects.find((p) => p.projectId === r.projectId)?.district || "",
        category:
          projects.find((p) => p.projectId === r.projectId)?.category || "",
        contractorName:
          projects.find((p) => p.projectId === r.projectId)?.contractorName ||
          "",
  ...
  res.json({
    ...
    data: {
      ...
      highRiskDetected: analysis.results.filter(
...
export async function getMpDirectory(
  req: Request,
  res: Response,
): Promise<void> {
  const { house, state, search } = req.query;
  const match: any = {};
  if (house && house !== "ALL") match.house = house;
  if (state && state !== "ALL") match.state = state;
  if (search) {
    const q = (search as string).trim();
    match.$or = [
      { mpName: { $regex: q, $options: "i" } },
      { constituency: { $regex: q, $options: "i" } },
      { state: { $regex: q, $options: "i" } },
    ];
  }

  const mps = await Project.aggregate([
    { $match: match },
    {
      $group: {
        _id: "$mpName",
  ...
  res.json({
    ...
    data: {
      ...
      mps: mps.map((m) => ({
        mpName: m.mpName,
        house: m.house || "Lok Sabha",
        state: m.state,
        constituency: m.constituency || "General",
        totalWorks: m.totalWorks,
        totalAllocated: m.totalAllocated,
        avgRiskScore: Math.round((m.avgRiskScore || 0) * 10) / 10,
      })),
    },
  });
}
