...
export const INDIAN_STATES_DISTRICTS = [
  ...
  },
  {
    ...
    districts: [
      ...
      { name: "North 24 Parganas", lat: 22.721, lon: 88.4819 },
...
export async function seedDemoAccounts(): Promise<void> {
  ...
  const collectorHash = await bcrypt.hash("Collector@12345", 10);
  ...
  const demoUsers = [
    ...
    {
      email: "collector@mplads.gov.in",
      name: "District Collector & DM",
      role: "AUDITOR",
      department: "District Planning & Execution Authority",
      designation: "District Magistrate / Collector",
      hash: collectorHash,
    },
    {
      email: "citizen@mplads.gov.in",
      name: "Public Citizen Observer",
      role: "VIEWER",
      department: "Public Grievance & Citizen Oversight",
  ...
  for (const u of demoUsers) {
    const { hash, ...userData } = u;
    await User.updateOne(
      { email: u.email },
      { $set: { ...userData, passwordHash: hash, isActive: true } },
      { upsert: true },
    );
  }
...
export async function seedFullDatabase(
  targetCount = 5200,
): Promise<{ projectsCount: number; anomaliesCount: number }> {
  console.log(
    `[Seed] Generating deterministic ${targetCount} synthetic MPLAD projects dataset...`,
  );
  ...
  const statuses: Array<
    "SANCTIONED" | "IN_PROGRESS" | "COMPLETED" | "DELAYED"
  > = ["IN_PROGRESS", "COMPLETED", "COMPLETED", "DELAYED", "SANCTIONED"];
  ...
  for (let i = 1; i <= targetCount; i++) {
    ...
    let utilizedAmount = Math.round(
      allocatedAmount * (status === "COMPLETED" ? 0.98 : 0.3 + (i % 6) * 0.1),
    );
    let progress =
      status === "COMPLETED"
        ? 100
    ...
    if (i % 45 === 0) {
      progress = 12 + (i % 12);
      utilizedAmount = Math.round(allocatedAmount * (0.9 + (i % 6) * 0.01));
      isGroundTruth = true;
    ...
    const startDate = new Date(
      approvalDate.getTime() + 25 * 24 * 60 * 60 * 1000,
    );
    ...
    const actualEnd =
      status === "COMPLETED"
        ? new Date(expectedEnd.getTime() + 20 * 24 * 60 * 60 * 1000)
        : undefined;
  ...
  console.log(
    "[Seed] Executing AI intelligence and anomaly detection pipeline on 5,000+ projects...",
  );
  ...
  console.log(
    "[Seed] Generating Anomaly Catalog and Sample Auditor Risk Cases...",
  );
  const highRiskProjects = projectsToInsert.filter(
  ...
  for (const hr of highRiskProjects) {
    ...
    if (caseCount < 25) {
      ...
      riskCaseDocs.push({
        ...
        findingsSummary:
          "Desk review initiated on preliminary cost deviation and contractor monopoly signals.",
        notes: [
          {
            ...
            authorRole: "AUDITOR",
            content:
              "Noticed significant cost outlier compared to district peer median. Requested measurement book from District Planning Office.",
            createdAt: new Date(),
          },
        ],
      });
    ...
    if (alertDocs.length < 15) {
      alertDocs.push({
        ...
        type:
          hr.signals[0]?.dimension === "CONTRACTOR"
  ...
  for (const c of contractors) {
    ...
    const totalVal = cProjects.reduce(
      (acc, curr) => acc + curr.allocatedAmount,
      0,
    );
    const utilizedVal = cProjects.reduce(
      (acc, curr) => acc + (curr.utilizedAmount || 0),
      0,
    );
    const hrCount = cProjects.filter(
      (p) => p.riskLevel === "HIGH" || p.riskLevel === "CRITICAL",
    ).length;
    const riskRate =
      cProjects.length > 0 ? Math.round((hrCount / cProjects.length) * 100) : 0;
    ...
    await Contractor.updateOne(
      { _id: c._id },
      {
        $set: {
          ...
          averageProjectValue:
            cProjects.length > 0 ? Math.round(totalVal / cProjects.length) : 0,
  ...
  for (const d of districts) {
    ...
    const totalAlloc = dProjects.reduce(
      (acc, curr) => acc + curr.allocatedAmount,
      0,
    );
    const totalUtil = dProjects.reduce(
      (acc, curr) => acc + (curr.utilizedAmount || 0),
      0,
    );
    const hrCount = dProjects.filter(
      (p) => p.riskLevel === "HIGH" || p.riskLevel === "CRITICAL",
    ).length;
    const avgScore =
      dProjects.length > 0
        ? Math.round(
            (dProjects.reduce((a, b) => a + b.riskScore, 0) /
              dProjects.length) *
              10,
          ) / 10
        : 0;
    ...
    await District.updateOne(
      { _id: d._id },
      {
        $set: {
          ...
          averageProjectCost:
            dProjects.length > 0
              ? Math.round(totalAlloc / dProjects.length)
              : 0,
  ...
  console.log(
    `[Seed] Seed completed successfully. Seeded ${projectsToInsert.length} projects, ${anomalyDocs.length} anomalies, and ${riskCaseDocs.length} risk cases.`,
  );
  return {
    projectsCount: projectsToInsert.length,
    anomaliesCount: anomalyDocs.length,
  };
}
